    ...
    if (isset($_SESSION['login_user3'])) { ?>
        ...
        <div class="tb flex-grow-1">
            ...
            if (isset($_POST['request'])) {
                $result = mysqli_query($conn, "select * from faculty_issue_book where bid = $_POST[b_id] AND faculty_username = '$_SESSION[login_user3]'")
                    or die("Failed to query database: " . mysqli_error($conn));
                $row = mysqli_fetch_array($result);
                $resu = mysqli_query($conn, "SELECT * FROM fileup INNER JOIN faculty_issue_book ON fileup.id = faculty_issue_book.bid AND faculty_issue_book.faculty_username='$_SESSION[login_user3]'");
            ...
            ?>
                        <script type="text/javascript">
                            ...
                            window.location = "faculty-view-book.php"
                        </script>
                    <?php
                    } else {

                        mysqli_query($conn, "INSERT INTO faculty_issue_book VALUES('$_SESSION[login_user3]','$_POST[b_id]','Pending','Pending','Pending','$_SESSION[fid]')");
                    ?>
                        <script type="text/javascript">
                    ...
                    ?>
                    <script type="text/javascript">
                        alert("You have already requested for five books! You can't request for more than three books!!")
                        window.location = "faculty-view-book.php"
                    </script>
            <?php
                }
